...
def random_mask(tokens, text_ids):
    """随机mask
    """
...
def sample_convert(tokens, text1, text2, label, random=False):
    """转换为MLM格式
    """
    ...
    if random:
        ...
        text1_ids, out1_ids = random_mask(tokens, text1_ids)
        text2_ids, out2_ids = random_mask(tokens, text2_ids)
    else:
...
class data_generator(DataGenerator):
    ...
    def __init__(self, data, batch_size=32, buffer_size=None, tokens=None):
        super(data_generator, self).__init__(data, batch_size=batch_size, buffer_size=buffer_size)
        self.tokens = tokens
    ...
    def __iter__(self, random=False):
        ...
        for is_end, (text1, text2, label) in self.sample(random):
            token_ids, segment_ids, output_ids = sample_convert(
                self.tokens, text1, text2, label, random
            )
...
class Evaluator(keras.callbacks.Callback):
    """评估与保存
    """
    def __init__(self, model, valid_generator, model_path='./'):
    ...
    def on_epoch_end(self, epoch, logs=None):
        val_score = evaluate(self.model, self.valid_generator)
        if val_score > self.best_val_score:
            self.best_val_score = val_score
            model_path = os.path.join(self.model_path, "best_model.weights")
